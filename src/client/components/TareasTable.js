...
const TareasTable = ({
  ...
  let tareasFiltered = tareasData.filter(tarea => permisos.find(permiso => permiso.area_id == tarea.area_id) || isAdmin);
  ...
  if (selectedAreaId > 0) {
    metasFilteredByArea = metas.filter(meta => meta.area_id == selectedAreaId)
  } else {
    metasFilteredByArea = metas;
  }
  if (selectedMetaId > 0) {
    metasFilteredByMeta = metasFilteredByArea.filter(meta => meta.id == selectedMetaId)
  } else {
    metasFilteredByMeta = metasFilteredByArea;
  }

  if (metas) {
    idsMeta = metasFilteredByMeta.filter(meta => permisos.find(permiso => permiso.area_id == meta.area_id) || isAdmin).map((meta) => (meta.id));
  }
  ...
  return (
    <Panel header={title} bsStyle="primary" eventKey="3">
      <BootstrapTable
        data={isAddingTarea ? [{estado_tarea:false}, ...tareasFiltered] : tareasFiltered}
        striped
        hover
